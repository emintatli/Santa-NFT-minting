    ...
    const connectHandler=async()=>{
      ...
      const mintHandler=async()=>{
        ...
        try{
          ...
          const tx=await nftData.contract.methods.awardItem(nftData.userWallet,amount).send({from:nftData.userWallet,gasPrice:"50000000000",value:amount*Web3.utils.toWei(nftPrice, 'ether')})
          if(tx){
      ...
      const psMintHandler=async()=>{
        ...
        try{
            setLoading(true);
          const tx=await nftData.contract.methods.awardItem(nftData.userWallet,amount).send({from:nftData.userWallet,gasPrice:"50000000000",value:amount=="1"?0:((amount-1)*Web3.utils.toWei(nftPrice, 'ether'))})
          if(tx){
            toast.success('🎅 Thanks for minting your Degen Santa.Your minted santa will appear in OpenSea within 5 - 10 minutes. If you haven\'t already, please go to opensea.io and link your metamask account. Thanks again, and have a very merry degen Xmas', {
              position: "bottom-right",
              hideProgressBar: false,
